...
/* LL gap: HPSYS_CFG SYSCR WDT reboot routing not covered by ll_wdt.h */
...
static inline bool wdt_sf32lb_is_enabled(const struct device *dev)
{
	...
	cr = config->wdt->WDT_CCR;

	return (cr == LL_WDT_CCR_FEED_VALUE);
}
...
static int wdt_sf32lb_disable(const struct device *dev)
{
	...
	config->wdt->WDT_CCR = WDT_CMD_STOP;
				      ...
				      const struct wdt_timeout_cfg *wdt_cfg)
{
	...
	config->wdt->WDT_CVR0 = (wdt_cfg->window.max * WDT_CLK_KHZ);
